...
def soft_delete_submitted_lead(current_user, submitted_lead_id):
    try:
        ...
        submitted_lead.email = submitted_lead.email + '::' + str(datetime.datetime.now()) + '::deleted' if submitted_lead.email else submitted_lead.email
        submitted_lead.deleted = 1
        insert_single_record(submitted_lead)
        return jsonify({'message': 'Successfully deleted..'}), 200
    except Exception as ex:
        return jsonify({'error': str(ex)}), 500


@submitted_lead_bp.route('/update/<submitted_lead_id>', methods=['PUT'])
@token_required
def update_submitted_lead(current_user, submitted_lead_id):
    try:
        ...
        for item in data:
            ...
            # Check if submitted_lead is already exist
            # if 'phone_num' in item and item['phone_num'] and submitted_lead.phone_num != item[
            #     'phone_num'] and is_submitted_lead_phone_num_exists(item['phone_num']):
            #     return {'error': 'Phone number is already exist.'}, 409
            # if 'alternate_phone_num' in item and item['alternate_phone_num'] and submitted_lead.alternate_phone_num != item[
            #     'alternate_phone_num'] and is_submitted_lead_alternate_phone_num_exists(item['alternate_phone_num']):
            #     return {'error': 'Alternate Phone number is already exist.'}, 409
            # if 'email' in item and item['email'] and submitted_lead.email != item['email'] and is_submitted_lead_email_exists(
            #         item['email']):
            #     return {'error': 'Email is already exist.'}, 409
            for key, value in item.items():
                setattr(submitted_lead, key, value)
            records_to_add.append(submitted_lead)
...
def get_paginated_submitted_leads_advanced(current_user):
    try:
        ...
        query = query.filter(model.SubmittedLead.agent_id == int(agent)) if agent else query
        print(query)
